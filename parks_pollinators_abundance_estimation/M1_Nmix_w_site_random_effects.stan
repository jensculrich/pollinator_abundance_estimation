...
parameters {
  ...
  vector[n_sites] nu;        // Random (site) effects on detection
  real<lower=0> sd_nu;         // Variation (scale) in (site) effects on detection
  
  vector[n_sites] xi;        // Random (site) effects on detection
  real<lower=0> sd_xi;         // Variation (scale) in (site) effects on detection
