...
# This R script calls a Stan program defines a Lincoln-Peterson mark-recapture model, 
...
stan_model <- stan_model("./mark_recapture.stan")
...
## Schnabel method
# For more than two visits, Schnabel:
# N_hat = SIGMA_t(M_t*C_t) / SIGMA_t(R_t + 1)

M_s <- c(32, 68)
C_s <- c(54, 37)
R_s <- c(18, 31)

my_dat <- c("M_s", "C_s", "R_s")

stan_model <- stan_model("./mark_recapture_schnabel.stan")
...
y <- vector(length = nrow(yaug))
for(i in 1:nrow(yaug)){
  y[i] <- sum(yaug[i, 1:T])
}
...
stan_model <- "./mark_recapture_data_augmentation.stan"
